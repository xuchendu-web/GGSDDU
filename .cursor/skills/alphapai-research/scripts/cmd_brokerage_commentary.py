...
def comment_list(
    ...
    subject_code: list[str] | None = None,
    subject_name: list[str] | None = None,
    ...
    return client._post(f"{COMMENTARY_PREFIX}/list", payload)
...
def event_list(
    ...
    if end_date:
        payload["endDate"] = end_date
    return client._post(f"{COMMENTARY_PREFIX}/event/list", payload)
...
def comment_detail(
    ...
    return client._post(f"{COMMENTARY_PREFIX}/detail", payload)
...
def register_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
    p = sub.add_parser(
        ...
        aliases=["review"],
        help="点评（brokerage commentary）：列表、详情、业绩点评专属列表",
    )
    ...
    p_elist.add_argument(
        ...
        required=True,
        choices=["ah", "a", "hk"],
        help="市场范围（必填）: ah=A+H a=A股 hk=港股",
    )
    p_elist.add_argument(
        "--event-type",
        nargs="+",
    ...
    p_detail.add_argument(
        "--comment-id", nargs="+", required=True, help="点评 ID（可传多个）"
    )
...
def run(args):
    ...
    if action == "list":
        ...
        print(f"[错误] 未知的 brokerage-commentary 子命令: {action}")
        return
